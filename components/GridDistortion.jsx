...
void main() {
  ...
  // Center and scale UV coordinates for cover effect
  uv = (uv - 0.5) * 1.0 + 0.5;
  
  vec4 offset = texture2D(uDataTexture, vUv);
  vec2 distortedUv = uv - 0.02 * offset.rg;
  
  // Clamp UV coordinates to prevent stretching
...
const GridDistortion = ({ grid = 15, mouse = 0.1, strength = 0.15, relaxation = 0.9, imageSrc, className = '' }) => {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      // Scale plane to cover container while maintaining image aspect ratio
