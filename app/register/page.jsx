...
import { submitProfileDetails, getcity, fetchstates, fetchProfileDetails, fetchSkills, fetchCertifications } from '@/utils/apihelper';
...
export default function ProfileDetails() {
    ...
    const handleSkillSelect = (skill) => {
        const val = (typeof skill === 'string') ? skill : (skill && (skill.skill_name || skill.name || skill.value)) || String(skill);
    ...
    const removeSkill = (skill) => {
        const key = (typeof skill === 'string') ? skill : (skill && (skill.skill_name || skill.name || skill.value)) || String(skill);
        setSelectedSkills(selectedSkills.filter(s => s !== key));
    };

    const handleCertificationSelect = (cert) => {
        const val = (typeof cert === 'string') ? cert : (cert && (cert.certificate_name || cert.name || cert.value)) || String(cert);
    ...
    const removeCertification = (cert) => {
        const key = (typeof cert === 'string') ? cert : (cert && (cert.certificate_name || cert.name || cert.value)) || String(cert);
    ...
    const normalizeToStringArray = (arr) => {
        ...
        const mapped = arr.map(item => {
            ...
            if (item == null) return '';
            // common name fields
            return item.skill_name || item.name || item.value || item.label || item.certificate_name || item.certificateName || JSON.stringify(item);
        }).filter(Boolean);
    ...
    useEffect(() => {
        ...
        const getProfile = async () => {
            ...
            if ((result && (result.status === true || result.success === true)) && result.data) {
                ...
                d = result.data;
            } else if (result && typeof result === 'object' && !('status' in result) && !('success' in result) && Object.keys(result).length) {
            ...
            setPersonalData({
                ...
                otp: '',
                whatsappAlerts: Boolean(d.whatsapp_alert ?? d.whatsappAlerts ?? d.whatsapp_alerts),
            });
            // Map educations -> front-end educationEntries shape
            const educationsRaw = Array.isArray(d.educations)
                ? d.educations
                : (typeof d.educations === 'string' ? tryParseJSON(d.educations) : []);
            const educationsMapped = (educationsRaw || []).map((ed) => ({
                ...
                visibility: ed.visibility || 'Public',
            }));

            // Map works -> front-end workEntries shape
            const worksRaw = Array.isArray(d.works)
                ? d.works
                : (typeof d.works === 'string' ? tryParseJSON(d.works) : []);
            ...
            const skillsMapped = Array.isArray(d.userSkills)
                ? d.userSkills.map((s) => s.skill_name || s.skillName || '').filter(Boolean)
                : (typeof d.userSkills === 'string' ? tryParseJSON(d.userSkills) : []);

            const certsMapped = Array.isArray(d.userCertificates)
                ? d.userCertificates.map((c) => c.certificate_name || c.certificateName || '').filter(Boolean)
                : (typeof d.userCertificates === 'string' ? tryParseJSON(d.userCertificates) : []);

            setEducationEntries(educationsMapped.length ? educationsMapped : educationEntries);
    ...
    const [personalErrors, setPersonalErrors] = useState({
        firstName: '', lastName: '', email: '', country: '', state: '', city: '', designation: '', mobile: '', otp: ''
    });
    ...
    const validatePersonal = () => {
        ...
        if (!personalData.lastName.trim()) errors.lastName = 'Last name is required';
        if (!personalData.email.trim()) errors.email = 'Email is required';
        else if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(personalData.email)) errors.email = 'Enter a valid email';
        if (!personalData.state.trim()) errors.state = 'State is required';
        if (!personalData.city.trim()) errors.city = 'City is required';
        if (!personalData.designation.trim()) errors.designation = 'Designation is required';
        if (!personalData.mobile.trim()) errors.mobile = 'Mobile is required';
        else if (!/^\d{10}$/.test(personalData.mobile.replace(/\D/g, ''))) errors.mobile = 'Enter a valid 10-digit mobile number';
    ...
    const validateEducation = () => {
        let errorsArr = educationEntries.map(entry => {
            ...
            if (!entry.educationType.trim()) err.educationType = 'Education type is required';
            if (!entry.marks.trim()) err.marks = 'Marks are required';
            if (!entry.fromDate) err.fromDate = 'From date is required';
            if (!entry.toDate) err.toDate = 'To date is required';
            return err;
        });
    ...
    const validateWork = () => {
        let errorsArr = workEntries.map(entry => {
            ...
            if (entry.experience !== 'Fresher') {
                if (!entry.designation.trim()) err.designation = 'Designation is required';
    ...
    const handlePersonalChange = (field, value) => {
        ...
        if (personalErrors[field]) setPersonalErrors({ ...personalErrors, [field]: '' });
    };
    ...
    const getExperienceDateRange = (experience, isToDate = false, fromDate = null) => {
        ...
        if (isToDate) {
            ...
            const maxToDate = new Date(fromDate.getFullYear() + years, fromDate.getMonth() + 11, fromDate.getDate());
            const minToDate = new Date(fromDate.getFullYear() + years, fromDate.getMonth() - 1, fromDate.getDate());
            ...
            const maxFromDate = new Date(today.getFullYear() - years, today.getMonth(), today.getDate());
    ...
    const handleSubmitProfile = async () => {
        ...
        console.log('Validation results:', { validPersonal, validEducation, validWork, validResume });
                    ...
                    {currentStep === 1 && (
                        <div className='max-w-xl mx-auto'>
                            <div className="text-center">
                                ...
                                <p className="text-gray-600 mb-6">Please provide all your details to proceed.</p>
                            </div>
                            ...
                            <div className="space-y-4 bg-white p-6 rounded-xl shadow">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-medium mb-2">Official Email Id</label>
                                        <input
                                            type="email"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        />
                                        {personalErrors.designation && <p className="text-red-500 text-xs mt-1">{personalErrors.designation}</p>}
                                    </div>
                                    <div>
                                        ...
                                        {personalErrors.mobile && <p className="text-red-500 text-xs mt-1">{personalErrors.mobile}</p>}
                                    </div>
                                </div>
                                ...
                                {otpSent && (
                                    <div>
                                        <p className="text-sm text-black mb-2">
                                            We have sent an OTP to your registered mobile number. It will be valid for the next 10 minutes. Please enter it in the box below.
                                        </p>
                                        <div className="relative">
                                            <input
                                                type="text"
                                                ...
                                                onChange={(e) => handlePersonalChange('otp', e.target.value)}
                                            />
                                            <button
                                                onClick={() => {
                                                    if (personalData.otp) {
                                        ...
                                        {personalErrors.otp && <p className="text-red-500 text-xs mt-1">{personalErrors.otp}</p>}
                                    </div>
                                )}
                                ...
                                <div className="flex items-center">
                                    <input
                                        ...
                                        onChange={(e) => setPersonalData({ ...personalData, whatsappAlerts: e.target.checked })}
                        ...
                        <div className='max-w-4xl mx-auto'>
                            ...
                            <div className="space-y-6 bg-white p-6 rounded-xl shadow">
                                {educationEntries.map((entry, index) => (
                                    <div key={index} className="space-y-4 border-b border-gray-200 pb-4 last:border-b-0">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div>
                                                ...
                                                />
                                                {educationErrors[index]?.institute && <p className="text-red-500 text-xs mt-1">{educationErrors[index].institute}</p>}
                                            </div>
                                            <div>
                                                ...
                                                />
                                                {educationErrors[index]?.educationType && <p className="text-red-500 text-xs mt-1">{educationErrors[index].educationType}</p>}
                                            </div>
                                            <div>
                                                ...
                                                />
                                                {educationErrors[index]?.marks && <p className="text-red-500 text-xs mt-1">{educationErrors[index].marks}</p>}
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div>
                                                ...
                                                <DatePicker
                                                    ...
                                                    onChange={(date) => handleEducationChange(index, 'fromDate', date)}
                                                    dateFormat="MMM yyyy"
                                                    showMonthYearPicker
                                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-prime focus:border-prime outline-none"
                                                    ...
                                                    maxDate={new Date()}
                                                />
                                                {educationErrors[index]?.fromDate && <p className="text-red-500 text-xs mt-1">{educationErrors[index].fromDate}</p>}
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium mb-2">To</label>
                                                <DatePicker
                                                    selected={entry.toDate}
                                                    onChange={(date) => handleEducationChange(index, 'toDate', date)}
                                                    dateFormat="MMM yyyy"
                                                    showMonthYearPicker
                                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-prime focus:border-prime outline-none"
                                                    placeholderText="Aug 2025"
                                                    wrapperClassName="w-full"
                                                    maxDate={new Date()}
                                                />
                                                {educationErrors[index]?.toDate && <p className="text-red-500 text-xs mt-1">{educationErrors[index].toDate}</p>}
                                            </div>
                                        ...
                                        <div>
                                            ...
                                            <textarea
                                                ...
                                                onChange={(e) => {
                                                    updateEducationEntry(index, 'description', e.target.value);
                                ...
                                <div>
                                    <label className="block text-sm font-medium mb-2">Visibility Settings (Who can see your education details):</label>
                                    <div className="flex gap-2 flex-col sm:flex-row sm:gap-4 md:gap-5">
                                        {['Public', 'Employers Only', 'Hidden'].map((option) => (
                                            <label key={option} className="flex items-center !text-gray-600">
                                                <input
                                                    ...
                                                    onChange={(e) => updateEducationEntry(0, 'visibility', e.target.value)}
                                                    className="mr-1"
                                                />
                                                {option}
                                            </label>
                            ...
                            <div className="space-y-6 bg-white p-6 rounded-xl shadow">
                                {workEntries.map((entry, index) => (
                                    <div key={index} className="space-y-4 border-b border-gray-200 pb-4 last:border-b-0">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div>
                                                <label className="block text-sm font-medium mb-2">{entry.experience === 'Fresher' ? 'Role/Designation' : 'Designation'}</label>
                                                <input
                                                    type="text"
                                                    placeholder={entry.experience === 'Fresher' ? 'Software Developer' : 'Social Media Marketing'}
                                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-prime focus:border-prime outline-none"
                                                    value={entry.designation}
                                                    onChange={(e) => handleWorkChange(index, 'designation', e.target.value)}
                                                />
                                                {workErrors[index]?.designation && <p className="text-red-500 text-xs mt-1">{workErrors[index].designation}</p>}
                                            </div>
                                            {entry.experience !== 'Fresher' && (
                                                <div>
                                                    ...
                                                    />
                                                    {workErrors[index]?.company && <p className="text-red-500 text-xs mt-1">{workErrors[index].company}</p>}
                                                </div>
                                            )}
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => updateWorkEntry(index, 'experience', e.target.value)}
                                                >
                                                    {experienceOptions.filter((option, optionIndex) => {
                                        ...
                                        {entry.experience !== 'Fresher' && (
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                <div>
                                                    ...
                                                    <DatePicker
                                                        selected={entry.fromDate}
                                                        onChange={(date) => handleWorkChange(index, 'fromDate', date)}
                                                        dateFormat="dd MMM yyyy"
                                                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-prime focus:border-prime outline-none"
                                                        placeholderText="04 May 2020"
                                                        wrapperClassName="w-full"
                                                        maxDate={getExperienceDateRange(entry.experience, false).maxDate}
                                                        showMonthDropdown
                                                        showYearDropdown
                                                        dropdownMode="select"
                                                    />
                                                    {workErrors[index]?.fromDate && <p className="text-red-500 text-xs mt-1">{workErrors[index].fromDate}</p>}
                                                </div>
                                                <div>
                                                    <label className="block text-sm font-medium mb-2">To</label>
                                                    <DatePicker
                                                        selected={entry.toDate}
                                                        onChange={(date) => handleWorkChange(index, 'toDate', date)}
                                                        dateFormat="dd MMM yyyy"
                                                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-prime focus:border-prime outline-none"
                                                        placeholderText="04 May 2020"
                                                        wrapperClassName="w-full"
                                                        minDate={getExperienceDateRange(entry.experience, true, entry.fromDate).minDate}
                                                        maxDate={getExperienceDateRange(entry.experience, true, entry.fromDate).maxDate}
                                                        showMonthDropdown
                                                        showYearDropdown
                                                        dropdownMode="select"
                                                    />
                                                    {workErrors[index]?.toDate && <p className="text-red-500 text-xs mt-1">{workErrors[index].toDate}</p>}
                                                </div>
                                                <div className="flex items-end">
                                        ...
                                        {entry.experience !== 'Fresher' && (
                                            <div>
                                                <label className="block text-sm font-medium mb-2">About Role</label>
                                                <textarea
                                                    ref={(el) => {
                                                        if (el && entry.description) {
                                                            el.style.height = 'auto';
                                                    ...
                                                    value={entry.description || ''}
                                                    onChange={(e) => {
                                                        updateWorkEntry(index, 'description', e.target.value);
                                                        e.target.style.height = 'auto';
                                                        e.target.style.height = Math.max(90, e.target.scrollHeight) + 'px';
                                                    }}
                                                />
                                ...
                                <div>
                                    <label className="block text-sm font-medium mb-2">Visibility Settings (Who can see your work experience details):</label>
                                    <div className="flex gap-2 flex-col sm:flex-row sm:gap-4 md:gap-5">
                                        {['Public', 'Employers Only', 'Hidden'].map((option) => (
                                            <label key={option} className="flex items-center">
                                                <input
                                                    ...
                                                    onChange={(e) => updateWorkEntry(0, 'visibility', e.target.value)}
                                                    className="mr-2"
                                                />
                                                {option}
                                            </label>
                        ...
                        <div className='max-w-4xl mx-auto'>
                            <div className="text-center p-6 bg-white rounded-xl shadow">
                                <div className="w-fit mx-auto">
                                    <Image src="/comp.png" width={240} height={240} alt="Congratulations" />
                                </div>
                                <h2 className="text-xl font-semibold">Profile Updated Successfully!!</h2>
                                <h3 className="text-xl font-semibold mb-4">Your profile information has been updated successfully.
                                </h3>
                                <p className="text-sm text-gray-600 mb-8">
                                    You can now continue exploring job opportunities that best match your skills and qualifications.
